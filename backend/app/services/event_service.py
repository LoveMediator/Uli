...
def create_event(
    ...
    db.commit()
    db.refresh(event)
    logger.info("event created event=%s user=%s rel=%s", event.public_id, user_id, rel.public_id)
    return event
...
def commit_a(
    ...
    db.refresh(event)
    db.refresh(snapshot)
    logger.info("snapshot_a committed event=%s user=%s", event.public_id, user_id)
...
def _execute_judge(
    ...
    try:
        ...
        db.rollback()
        raise AppError("裁判已生成，请勿重复操作", code=INVALID_STATE) from exc
...
def _execute_judge_inner(
    ...
    logger.info(
        "judge completed event=%s judge=%s operator=%s",
        event.public_id,
        judge_result.public_id,
        operator_user_id,
    )
    return judge_result
