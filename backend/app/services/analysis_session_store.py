...
class AnalysisSessionStore:
    def __init__(self, client: Redis[str], ttl_seconds: int) -> None:
        self._client = client
    ...
    def _refresh_indexes(self, session: dict[str, Any]) -> None:
        relationship_public_id = session.get("relationshipId")
        ...
        event_public_id = session.get("eventId")
        if event_public_id:
        ...
        if scope_id:
            scope_key = self._scope_index_key(session["phase"], scope_id, int(session["userId"]))
    ...
    def get_session(self, session_id: str) -> dict[str, Any] | None:
        try:
            session = _json_loads(self._client.get(self._session_key(session_id)))
    ...
    def get_scoped_session(self, scope: str, scope_id: str, user_id: int) -> dict[str, Any] | None:
        try:
            session_id = self._client.get(self._scope_index_key(scope, scope_id, user_id))
            if not session_id:
                return None
            session = _json_loads(self._client.get(self._session_key(session_id)))
            if session is None:
    ...
    def clear_sessions_for_relationship(self, relationship_public_id: str) -> None:
        try:
            relationship_key = self._relationship_set_key(relationship_public_id)
            session_ids = [session_id for session_id in self._client.smembers(relationship_key) if session_id]
            for session_id in session_ids:
                ...
                if scope_id:
                    self._client.delete(
                        self._scope_index_key(session["phase"], scope_id, int(session["userId"]))
                    )
                event_public_id = session.get("eventId")
                if event_public_id:
