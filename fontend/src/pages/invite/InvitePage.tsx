...
export function InvitePage() {
  ...
  const inviteQuery = useQuery({
    ...
    queryFn: () => eventsApi.getInvite(eventId),
    enabled: !!eventId,
  });

  const snapshotQuery = useQuery({
    queryKey: ['snapshot-a', eventId],
    queryFn: () => eventsApi.getSnapshotA(eventId),
    enabled: isAuthenticated && !!eventId,
  });

  const agreeMutation = useMutation({
    mutationFn: () => eventsApi.bAgree(eventId, { agree: true }),
    onSuccess: () => setResultReady(true),
  });

  const commitBMutation = useMutation({
    mutationFn: () =>
      eventsApi.commitB(eventId, {
  ...
  const judgeQuery = useQuery({
    ...
    queryFn: () => eventsApi.getJudgeResult(eventId),
    enabled:
      !!eventId &&
      isAuthenticated &&
