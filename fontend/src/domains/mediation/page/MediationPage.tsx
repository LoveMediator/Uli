...
import { Button, Card, Input, LoadingSpinner, Modal, Textarea } from '@/shared/ui';
...
export function MediationPage() {
  ...
  const createEventMutation = useMutation({
    mutationFn: () => mediationApi.createEvent({ title: title.trim(), relationshipId }),
    onSuccess: (data) => {
      setCurrentEvent({
  ...
  const commitMutation = useMutation({
    mutationFn: () => mediationApi.commitA(currentEvent!.eventId, { confirmText: confirmText.trim() }),
    onSuccess: (data) => {
      patchCurrentEvent({
        status: data.status,
    ...
    <div className="relative flex h-full flex-col bg-milk-50">
          ...
          <Card className="space-y-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-coffee-800/70">
                会调用真实的 `POST /events`，默认关系 ID：{relationshipId}
              </p>
            </div>
            <Input
              label="事件标题"
            ...
            />
            {createEventMutation.error ? <p className="text-sm font-semibold text-red-400">{getErrorMessage(createEventMutation.error)}</p> : null}
            <Button
              fullWidth
      ...
      <Modal open={analysisPreviewOpen} onClose={() => setAnalysisPreviewOpen(false)}>
        <Card className="space-y-4 rounded-[32px]">
          ...
          {commitMutation.error ? <p className="text-sm font-semibold text-red-400">{getErrorMessage(commitMutation.error)}</p> : null}
          <div className="flex gap-3">
            <Button
              fullWidth
              variant="secondary"
